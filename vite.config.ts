...
export default defineConfig(async () => ({
  ...
  test: {
    // Playwright 用例由独立浏览器运行器执行，不能被 Vitest 当作普通 spec 重复收集。
    ...
    rollupOptions: {
      output: {
        ...
        manualChunks(id) {
          ...
          // Vditor 是正式编辑器但仍由 Workspace 懒加载；Milkdown 只服务 /editor-lab 对照页。
          if (id.includes("vditor")) return "editor-vditor-vendor";
          if (id.includes("@milkdown")) return "editor-lab-milkdown-vendor";
