...
test.describe("六项反馈浏览器回归", () => {
  ...
  test("全局搜索的范围、标签、目录筛选组合在浏览器中生效", async ({ page }) => {
    ...
    await expect(page.locator(".global-search-summary")).toContainText("找到 1 篇")
    await query.fill('title:"搜索结果 002" tag:计划 公共检索词')
    await expect(page.locator(".global-search-summary")).toContainText("找到 1 篇")
    await expect(page.locator(".global-search-results [role=option]")).toContainText(["搜索结果 002"])
    await query.fill('title:"搜索结果 003" tag:计划 公共检索词')
    await expect(page.locator(".global-search-summary")).toContainText("找到 0 篇")
    await query.fill("tag:计划 -title:003 公共检索词")
    await expect(page.locator(".global-search-summary")).toContainText("找到 1 篇")
  ...
  test("保存搜索在重开后恢复完整筛选，并可删除", async ({ page }) => {
    await seedRegressionVault(page)
    await page.getByRole("button", { name: /全局搜索/ }).first().click()
    const search = page.getByRole("combobox", { name: "全局搜索笔记" })
    await search.fill("公共检索词 -body:草稿")
    await page.getByLabel("搜索范围").selectOption("body")
    await page.getByLabel("筛选标签").selectOption("计划")
    await page.getByLabel("筛选目录").selectOption("目标")
    await page.getByRole("button", { name: "保存当前搜索" }).click()
    await page.getByLabel("搜索名称").fill("计划正文")
    await page.getByRole("button", { name: "确定" }).click()
    await expect(page.getByLabel("已保存搜索", { exact: true })).toHaveValue("计划正文")
    await page.getByRole("button", { name: "关闭全局搜索" }).click()
    await page.reload()
    await page.getByRole("button", { name: /全局搜索/ }).first().click()
    await page.getByLabel("已保存搜索", { exact: true }).selectOption("计划正文")
    await expect(search).toHaveValue("公共检索词 -body:草稿")
    await expect(page.getByLabel("搜索范围")).toHaveValue("body")
    await expect(page.getByLabel("筛选标签")).toHaveValue("计划")
    await expect(page.getByLabel("筛选目录")).toHaveValue("目标")
    await expect(page.locator(".global-search-summary")).toContainText("找到 1 篇")
    await page.getByRole("button", { name: "删除已保存搜索" }).click()
    await expect(page.getByLabel("已保存搜索", { exact: true }).locator("option")).toHaveCount(1)
  })
