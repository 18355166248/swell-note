...
test("本地批量移动保留附件链接、标签与回收站恢复", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "desktop-chrome")
  await installMockVaultPicker(page, { ...sourceFiles, "归档/占位.md": "# 占位" })
  await page.goto("/#/notes")
  await page.getByRole("button", { name: "打开本地库" }).click()
  await expect(page.getByText("占位", { exact: true }).first()).toBeVisible()
  ...
  const snapshot = () => page.evaluate(() => (window as unknown as { __backupMock: { snapshot: (name: string) => Record<string, number[]> } }).__backupMock.snapshot("source"))
  const moved = await snapshot()
  expect(new TextDecoder().decode(new Uint8Array(moved["归档/第一篇.md"]))).toContain("../attachments/chart.png")
  expect(new TextDecoder().decode(new Uint8Array(moved["归档/第二篇.md"]))).toContain("tags: [计划]")
  expect(moved["项目/第一篇.md"]).toBeUndefined()
  await page.getByRole("button", { name: "批量整理笔记" }).filter({ visible: true }).click()
  ...
  await expect(dialog).toContainText("已完成 2 / 2 篇")
  await dialog.getByRole("button", { name: "完成", exact: true }).click()
  expect(Object.keys(await snapshot()).filter((path) => path.startsWith(".swell-trash/"))).toHaveLength(2)
  ...
  await expect(page.getByText("回收站为空", { exact: false })).toBeVisible()
  const restored = await snapshot()
  expect(restored["归档/第一篇.md"]).toEqual(moved["归档/第一篇.md"])
  expect(restored["归档/第二篇.md"]).toEqual(moved["归档/第二篇.md"])
})
