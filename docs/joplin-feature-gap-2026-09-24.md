...
进展：备份完整性核对、恢复前预览、单篇笔记标签编辑、本地与 WebDAV 工作副本的跨笔记标签重命名、组合搜索筛选已实施。缺少笔记正文或引用附件时会停止 ZIP 下载并显示清单；恢复前显示 ZIP 文件、已知同名笔记和清单数量异常；标签写回 Markdown frontmatter。仍需真实 Vault 的端到端恢复演练。
...
| 搜索 | 本地 SQLite FTS / Web 缓存索引、全局标题/正文/标签/目录筛选、`title:`/`tag:` 与 `-body:` 等排除限定词 | 已具备基础组合搜索；尚无保存搜索等完整语法 |
...
5. **组合搜索与快速跳转。** 全局搜索现可组合范围、目录、单标签、更新时间与收藏状态，并支持 `title:`、`tag:` 及 `-title:`、`-tag:`、`-body:` 限定词；正文排除仅纳入已加载或确实存在于缓存的正文，缓存查询失败时会提示结果可能不完整。尚无保存搜索等完整语法；仍需实测中文大库的性能和召回。Joplin 提供丰富查询语法和 Goto Anything。
