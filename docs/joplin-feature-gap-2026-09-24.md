...
进展：本报告提出的“备份完整性结果”已开始实施。当前代码会在缺少笔记正文或引用附件时停止 ZIP 下载，并在存储页显示缺失清单；仍需真实 Vault 的端到端恢复演练。
...
4. **标签管理。** 目前能解析 frontmatter 标签、显示并筛选，但 UI 未见新增/重命名/批量赋标签入口。先做笔记级标签编辑和批量操作，再考虑标签管理页；直接修改 Markdown frontmatter，保留未知字段与原有正文。
5. **组合搜索与快速跳转。** 当前全局搜索有范围、目录、单标签筛选，但未见 `title:`、`tag:`、日期、排除词、精确短语等组合语法。建议先支持 4–5 个高频过滤器与保存搜索，并提供清晰的搜索范围/索引未就绪提示；中文搜索需实测大库性能和召回。Joplin 提供丰富查询语法和 Goto Anything。
