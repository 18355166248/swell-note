...
桌面端与 iOS 的后续核对见 [分端功能续查](joplin-desktop-ios-gap-2026-09-24.md)：补充系统分享、扫描/录音、应用锁、附件预下载、导出格式及原生文件验收，并记录正向 `body:` 搜索的语义缺口。备份进度以演练记录的“第三轮桌面端验收”为准：不完整 ZIP 已通过 macOS 隔离恢复，仍有 6 个缺失附件，iOS 尚待验收。

进展：备份完整性核对、恢复前预览、单篇笔记标签编辑、本地与 WebDAV 工作副本的跨笔记标签重命名、组合搜索筛选和当前设备保存搜索已实施。缺少笔记正文或引用附件时会停止 ZIP 下载并显示清单；恢复前显示 ZIP 文件、已知同名笔记和清单数量异常；标签写回 Markdown frontmatter。仍需真实 Vault 的端到端恢复演练。

补充实施：正向 `body:` 搜索、原生 ZIP 保存路径与 iOS 原位目录参数已修正；桌面/移动端已增加批量移动、回收站删除和标签添加/移除。见 [本轮实施记录](batch-organize-2026-09-24.md)。
...
| 搜索 | 本地 SQLite FTS / Web 缓存索引、全局标题/正文/标签/目录筛选、`title:`/`tag:` 与 `-body:` 等排除限定词、当前设备按库保存搜索 | 已具备基础组合搜索；保存搜索暂不跨设备同步，查询语法仍不完整 |
| 历史、回收站、备份 | 本地版本历史与逐行差异、回收站、整库备份恢复 | 已具备；历史仅当前设备保留最多 30 个版本，整库 ZIP 不包含历史版本 |
...
3. **备份完整性结果。** 原实现允许跳过不可读笔记/附件，部分情况仍返回成功。现已改为导出前核对并阻止不完整 ZIP，恢复前显示文件清单与异常提示。真实本地笔记库的服务层演练已确认 88 篇笔记和 1 个可读取附件的 ZIP 往返哈希一致，但发现 9 个不可读取的附件引用，完整备份门槛未通过；独立测试库的桌面浏览器应用端导出、空库恢复和哈希比对已通过。见 `docs/backup-rehearsal-2026-09-24.md`。修复真实库引用后仍需重跑，并补 Tauri 真机恢复验收。

### P1：完善笔记整理、找回与迁移的闭环

4. **标签管理。** 已有笔记级 frontmatter 标签编辑、显示和筛选；可写本地 Vault 和 WebDAV 离线工作副本已支持从标签筛选菜单跨笔记重命名，分别核对磁盘版本和缓存编辑序号并报告跳过项。批量赋标签和标签管理页仍待补齐。继续以 Markdown frontmatter 为事实来源，保留未知字段与原有正文。
5. **组合搜索与快速跳转。** 全局搜索现可组合范围、目录、单标签、更新时间与收藏状态，并支持 `title:`、`tag:` 及 `-title:`、`-tag:`、`-body:` 限定词；正文排除仅纳入已加载或确实存在于缓存的正文，缓存查询失败时会提示结果可能不完整。保存搜索包含上述筛选条件，按笔记库保存在当前设备；仍需实测中文大库的性能和召回，并评估跨设备同步。Joplin 提供丰富查询语法和 Goto Anything。
