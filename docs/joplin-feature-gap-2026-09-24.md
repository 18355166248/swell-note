...
| 搜索 | 本地 SQLite FTS / Web 缓存索引、全局标题/正文/标签/目录筛选、`title:`/`tag:` 限定词 | 已具备基础组合搜索；尚无排除词、保存搜索等完整语法 |
...
5. **组合搜索与快速跳转。** 全局搜索现可组合范围、目录、单标签、更新时间与收藏状态，并支持 `title:`、`tag:` 限定词；缓存正文查询失败时会提示结果可能不完整。尚无排除词、保存搜索等完整语法；仍需实测中文大库的性能和召回。Joplin 提供丰富查询语法和 Goto Anything。
