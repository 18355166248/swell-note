...
3. **备份完整性结果。** 原实现允许跳过不可读笔记/附件，部分情况仍返回成功。现已改为导出前核对并阻止不完整 ZIP，恢复前显示文件清单与异常提示。真实本地笔记库的服务层演练已确认 88 篇笔记和 1 个可读取附件的 ZIP 往返哈希一致，但发现 9 个不可读取的附件引用，完整备份门槛未通过；见 `docs/backup-rehearsal-2026-09-24.md`。修复引用后仍需重跑，并补应用界面/Tauri 真实恢复验收。
