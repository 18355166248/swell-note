...
进展：备份完整性核对、恢复前预览、单篇笔记标签编辑、本地 Vault 跨笔记标签重命名和组合搜索筛选已实施。缺少笔记正文或引用附件时会停止 ZIP 下载并显示清单；恢复前显示 ZIP 文件、已知同名笔记和清单数量异常；标签写回 Markdown frontmatter。仍需真实 Vault 的端到端恢复演练。
...
4. **标签管理。** 已有笔记级 frontmatter 标签编辑、显示和筛选；可写本地 Vault 已支持从标签筛选菜单跨笔记重命名，逐篇核对磁盘版本并报告跳过项。WebDAV 跨笔记重命名、批量赋标签和标签管理页仍待补齐。继续以 Markdown frontmatter 为事实来源，保留未知字段与原有正文。
