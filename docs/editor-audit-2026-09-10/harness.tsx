...
import { writeVaultAttachments, type AttachmentWriter } from '../../src/services/vault/attachment-writer'
import type { VaultAsset } from '../../src/services/vault/vault-adapter'
import '../../src/App.css'
...
function Audit() {
  ...
  const assets = useRef(new Map<string, VaultAsset>())
  const memoryWriter = useRef<AttachmentWriter>({
    async createBinaryFile(path, data, mimeType) {
      if (assets.current.has(path)) throw new Error(`文件已存在：${path}`)
      assets.current.set(path, { data: data.slice(), mimeType })
      return { path }
    },
  })
  ...
  const insertMemoryFiles = async (files: File[], position?: number) => {
    if (busyRef.current) { setAttachError('上一批附件仍在写入，请完成后再试'); return }
    const insertion = ref.current?.captureInsertion(position)
    busyRef.current = true
    setBusy(true)
    setAttachError(null)
    try {
      // 直接复用生产附件 writer：命名、Markdown 语法、分段边界均与真实工作区一致，只有存储落在内存 Map。
      const result = await writeVaultAttachments(memoryWriter.current, 'audit.md', files)
      if (result.markdown && !insertion?.insert(result.markdown)) throw new Error('编辑器已卸载，插入被取消')
      if (result.errors.length > 0) setAttachError(result.errors.join('；'))
    } catch (error) {
      setAttachError(error instanceof Error ? error.message : '插入附件失败')
    } finally {
      insertion?.dispose()
  ...
  const resolveMemoryAsset = async (source: string) => assets.current.get(source) ?? null
  return <main style={{ maxWidth: 1100, margin: '24px auto', padding: 20 }}>
    ...
    <button onClick={() => { assets.current.clear(); setValue(seed); setFormat(null); setTable(false); setSelection(false); setHistory([false, false]); setGeneration(g => g + 1) }}>载入样本</button>
    ...
    <section className="note-editor" style={{ border: '1px solid #aaa', marginTop: 12 }}>
      <FormattingToolbar editorRef={ref} onFormat={text => ref.current?.insertText(text)} onInsertFiles={insertMemoryFiles} canInsertAttachment attachmentBusy={busy} canUndo={history[0]} canRedo={history[1]} formatState={format} editingTable={table} hasSelection={selection} />
      <div className="markdown-editor-shell">
        <MarkdownEditor key={generation} ref={ref} storageKey={`audit-${generation}`} value={value} onChange={setValue} onInsertFiles={insertMemoryFiles} onResolveAsset={resolveMemoryAsset} onPasteError={setAttachError} onFormatStateChange={setFormat} onEditingTargetChange={setTable} onSelectionChange={setSelection} onHistoryChange={(u, r) => setHistory([u, r])} />
      </div>
    </section>
