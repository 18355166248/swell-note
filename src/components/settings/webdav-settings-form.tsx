    ...
    <form className="settings-content-card webdav-settings-form" onSubmit={(event) => void handleConnect(event)}>
      ...
      <div className="settings-form-grid">
        ...
        <div className="settings-field">
          ...
          <Input autoComplete="username" id="webdav-username" name="username" onChange={(event) => updateConfig({ username: event.target.value })} spellCheck={false} type="email" value={config.username} />
        </div>
        <div className="settings-field">
          ...
          <div className="settings-password-field">
            <Input autoComplete="current-password" id="webdav-password" name="password" onChange={(event) => { setSaved(false); setConnectionError(""); setPassword(event.target.value) }} placeholder="可由密码管理器自动填充" type={passwordVisible ? "text" : "password"} value={password} />
          ...
          <p className="settings-security-note"><KeyRound />Swell Note 不保存密码；刷新后可在当前笔记页快速重连。</p>
        </div>
        <div className="settings-field">
